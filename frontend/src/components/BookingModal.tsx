...
export default function BookingModal({ isOpen, onClose, equipmentId }: any) {
  ...
  const isSelected = (day: number) => selectedDate === `${calYear}-${String(calMonth + 1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
  ...
  const handleBooking = async () => {
    ...
    try {
      const res = await fetch(`${API}/bookings`, {
        ...
        body: JSON.stringify({ equipmentId, date: selectedDate, startTime, duration, paymentMethod }),
      });
  ...
  return (
    // Portal renders this directly into document.body —
    // completely outside any stacking context created by backdrop-filter/transform
    <Portal>
      <div
        style={{ position: "fixed", inset: 0, zIndex: 99999 }}
        className="bg-black/50 flex items-center justify-center px-4"
      ...
      >
        <div className="bg-[#FDF6E3] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div className="flex items-center justify-between px-6 pt-6 pb-4">
            ...
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#E5D5B5] transition text-[#5C3D1E]">
              <X className="w-5 h-5" />
            </button>
          </div>
            ...
            <div>
              ...
              <div className="bg-white rounded-2xl border border-[#E5D5B5] p-4">
                <div className="flex items-center justify-between mb-4">
                  <button onClick={prevMonth} className="w-9 h-9 flex items-center justify-center rounded-xl border border-[#E5D5B5] hover:bg-[#F5EDD8] transition">
                    <ChevronLeft className="w-4 h-4 text-[#5C3D1E]" />
                  </button>
                  <span className="font-bold text-[#2C1A0E] text-sm">{MONTHS[calMonth]} {calYear}</span>
                  <button onClick={nextMonth} className="w-9 h-9 flex items-center justify-center rounded-xl border border-[#E5D5B5] hover:bg-[#F5EDD8] transition">
                ...
                <div className="grid grid-cols-7 mb-2">
                  {DAYS.map(d => (
                    <div key={d} className="text-center text-xs font-semibold text-[#5C3D1E]/60 py-1">{d}</div>
                  ))}
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`
                          ...
                          ${past ? "text-[#5C3D1E]/25 cursor-not-allowed" : "cursor-pointer"}
                          ${sel  ? "bg-[#E6A817] text-white hover:bg-[#D4981A]" : ""}
                          ${tod && !sel ? "border-2 border-[#E6A817] text-[#2C1A0E]" : ""}
                          ${!sel && !tod && !past ? "text-[#2C1A0E] hover:bg-[#F5EDD8]" : ""}
                        `}
                      >
                        {day}
                      </button>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <label key={opt.value} className="flex items-center gap-3 cursor-pointer" onClick={() => setPaymentMethod(opt.value)}>
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition
                      ${paymentMethod === opt.value ? "border-[#4A2E15] bg-[#4A2E15]" : "border-[#C9A96E] bg-white"}`}>
